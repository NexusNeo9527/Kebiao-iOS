...
enum ScheduleImportService {
    ...
    static func parsePDF(data: Data, sourceName: String = "课表.pdf") throws -> ScheduleImportPreview {
        ...
        if !text.isEmpty && text.filter({ $0 == "\u{FFFD}" }).count <= 3 {
            if let parsed = try? parseSchoolText(text, sourceName: sourceName) {
                return ScheduleImportPreview(sourceName: sourceName, format: .pdf, courses: parsed.courses, warnings: parsed.warnings)
            }
            let loose = courses(fromLoosePDFText: text)
            if !loose.0.isEmpty {
                return ScheduleImportPreview(sourceName: sourceName, format: .pdf, courses: loose.0, warnings: loose.1)
            }
        }

        let recognized = recognizePDFPages(document)
        guard !recognized.0.isEmpty else {
            throw ScheduleImportError.malformed("未能从 PDF 识别课程。请确认页面包含清晰的星期、节次和课程名称")
        }
        return ScheduleImportPreview(
            sourceName: sourceName,
            format: .pdf,
            courses: recognized.0,
            warnings: ["已使用图像识别，请核对课程名、星期、节次和周次"] + recognized.1
        )
    }
